...
import { User, UserAccount } from "./users.model";
...
export const setUserAccountType = async (userCode: string, accountType: AccountTypes) => {
  try {
    let result = await connector
      .table(userTable)
      .update({
...
export const getUsers = async (pageSize?: number, pageNumber?: number, dateOrder?: string): Promise<User[]> => {
  try {
    ...
    let procResult: User[] = sanitize(await result);
    return procResult;
  } catch (error) {
    logger.error(error.message);
    throw new DatabaseError("Internal connection error");
...
export const getUsersAndAccounts = async (pageSize?: number, pageNumber?: number, dateOrder?: string): Promise<UserAccount[]> => {
  try {
    let result = connector.table(userTable).select("*").join(authenticationTable, { "users.user_code": "authentication.user_code" });
    if (pageSize != null && pageNumber != null) {
      result = result.limit(pageSize).offset(pageNumber);
    }
    if (dateOrder != null) {
    ...
    let procResult: any[] = sanitize(await result);
    procResult.map((user) => {
      delete user.wakanda;
    ...
    let accounts: UserAccount[] = procResult;
    return accounts;
  } catch (error) {
    logger.error(error.message);
    throw new DatabaseError("Internal connection error");
